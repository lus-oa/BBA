...
def simulatePlay(BandWidth):
    ...
    for currentTime in indexList:
        ...
        if bytesLeft >= VrateTmp * toRecv:  # 判断当前网络带宽能否以选择的速率下载完一块
            ...
            # 动态计算蓄水池大小
            avgRate = VrateCheck(VrateList, numpy.mean(SelectedRateList))
            deltareservior = int(currentBW - VrateTmp) * 480 / avgRate
...
x = range(startPoint, endPoint)
plt.plot(x, SelectedRateList[startPoint:endPoint], label='BBA-2 rate', c='blue', marker='^')
